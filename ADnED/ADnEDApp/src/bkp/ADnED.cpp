...
ADnED::ADnED(const char *portName, int maxBuffers, size_t maxMemory, int debug)
  : ADDriver(portName,
    ...
    m_debug(debug),single_file_num(0),capture_file_num(0),pulse_id_dim(1), capture_group_num(0),storageFlag(1),stream_group_num(0)
{

  // <mine
  GEM_HV1_len = strlen(GEM_HV_1);
  GEM_HV2_len = strlen(GEM_HV_2);
  GEM_gas_len = strlen(GEM_gas);
  //  mine>  
...
void ADnED::eventHandler(shared_ptr<epics::pvData::PVStructure> const &pv_struct, epicsUInt32 channelID)
{
  ...
  if (pixelsPtr && tofPtr) {
    ...
    for(int list_num_pixel = 0; list_num_pixel<pixelsLength; list_num_pixel++){
          pixelsDataCopy[list_num_pixel] = pixelsData[list_num_pixel];
          tofDataCopy[list_num_pixel] = tofData[list_num_pixel];
        }  
   
    sprintf(stream_pixel_name, "pixel_%5.5d", stream_group_num);
    sprintf(stream_tof_name, "tof_%5.5d", stream_group_num);

    sprintf(capture_pixel_name, "pixel_%5.5d", capture_group_num);
    //cout << "capture_tof_name::" << capture_tof_name<<endl;
    sprintf(capture_tof_name, "tof_%5.5d", capture_group_num);
    ...
    cout << "storageFlag:: "<< storageFlag<<endl; 
    ...
    switch(hdfFileWriteMode){
      ...
      case 1:
        ...
        NXmakedata (capture_file_id, "GEM_HV_1", NX_CHAR, 1, &GEM_HV1_len);
        NXopendata (capture_file_id, "GEM_HV_1");
        NXputdata (capture_file_id, GEM_HV_1);
        NXclosedata (capture_file_id);
        
        NXmakedata (capture_file_id, "GEM_HV_2", NX_CHAR, 1, &GEM_HV2_len);
        NXopendata (capture_file_id, "GEM_HV_2");
        NXputdata (capture_file_id, GEM_HV_2);
        NXclosedata (capture_file_id);

        NXmakedata (capture_file_id, "GEM_gas", NX_CHAR, 1, &GEM_gas_len);
        NXopendata (capture_file_id, "GEM_gas");
        NXputdata (capture_file_id, GEM_gas);
        NXclosedata (capture_file_id);

        capture_file_num++;
        }
        //cout<< "the capture_file_num ::" << capture_file_num << "    "<< single_hdf_file_name <<endl;
        
          NXmakegroup (capture_file_id, "data", "NXdata");
          NXopengroup (capture_file_id, "data", "NXdata");
              
            /* out the pulse id data  */         
            // NXmakedata (capture_file_id, "pulse_ID", NX_UINT64, 1 , &pulse_id_dim);
            // NXopendata (capture_file_id, "pulse_ID");
            // NXputdata (capture_file_id, pulseIDCopy);
            // //NXputattr (capture_file_id, "units", " ", 7, NX_CHAR);
            // NXclosedata (capture_file_id); 
                
            /* Output the pxiel ID */
            NXmakedata (capture_file_id, capture_pixel_name, NX_UINT64, 1 , &dim);
            NXopendata (capture_file_id, capture_pixel_name);
            NXputdata (capture_file_id, pixelsDataCopy);
            //NXputattr (capture_file_id, "units", " ", 7, NX_CHAR);
            NXclosedata (capture_file_id);
                
            NXmakedata (capture_file_id, capture_tof_name, NX_UINT64, 1, &dim);
            NXopendata (capture_file_id, capture_tof_name);
            NXputdata (capture_file_id, tofDataCopy);
      ...
      case 2:
        ...
        { 
          ...
          if (nxstat == NX_ERROR) {
           ...
           NXmakedata (stream_file_id, "GEM_HV_2", NX_CHAR, 1, &GEM_HV2_len);
           NXopendata (stream_file_id, "GEM_HV_2");
              NXputdata (stream_file_id, GEM_HV_2);
           NXclosedata (stream_file_id);

             NXmakedata (stream_file_id, "GEM_gas", NX_CHAR, 1, &GEM_gas_len);
           NXopendata (stream_file_id, "GEM_gas");
              NXputdata (stream_file_id, GEM_gas);
           NXclosedata (stream_file_id);

           NXmakedata (stream_file_id, "GEM_HV_1", NX_CHAR, 1, &GEM_HV1_len);
           NXopendata (stream_file_id, "GEM_HV_1");
              NXputdata (stream_file_id, GEM_HV_1);
           NXclosedata (stream_file_id);
           
         }

        // nxstat= NXreopen (stream_file_id, &stream_clone_fileid);
        ...
        NXopengroup (stream_file_id, "data", "NXdata");
          ...
          // NXmakedata (stream_file_id, "pulse_ID", NX_UINT64, 1 , &pulse_id_dim);
          // NXopendata (stream_file_id, "pulse_ID");
          // NXputdata (stream_file_id, pulseIDCopy);
          //         //NXputattr (stream_file_id, "units", " ", 7, NX_CHAR);
          // NXclosedata (stream_file_id); 
              
          /* Output the pxiel ID */
          NXmakedata (stream_file_id, stream_pixel_name, NX_UINT64, 1 , &dim);
          NXopendata (stream_file_id, stream_pixel_name);
          NXputdata (stream_file_id, pixelsDataCopy);
          NXclosedata (stream_file_id);
          NXflush (&stream_file_id);      
  
          NXmakedata (stream_file_id, stream_tof_name, NX_UINT64, 1, &dim);
          NXopendata (stream_file_id, stream_tof_name);
          NXputdata (stream_file_id, tofDataCopy);
          NXputattr (stream_file_id, "units", "microseconds", 12, NX_CHAR);
          NXclosedata (stream_file_id);
          NXflush (&stream_file_id);      
